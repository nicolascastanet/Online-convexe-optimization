...
class SERG():
    ...
    def train(self,epoch,radius,X, train_A, test_A, train_B, test_B,alpha=0, proj=True):
        ...
        for t in train_samples:
            # pick an expert
            expert = np.random.choice(np.arange(dim), 1, p=(W[:dim]+W[dim:]))
            ...
            # Update parameters depending on the choosen expert
            if grad[expert] > 0:
                W[expert] = np.exp(-(step*grad[expert])/W[expert])*W[expert]
            else:
                W[expert+dim] = np.exp((step*grad[expert])/W[expert+dim])*W[expert+dim]
